...
- **Market info (Binance Futures):** `GET /api/futures/market-info?symbol=BTC` (server proxy to premiumIndex)
- **Technical indicators:** `GET /api/ta/indicator?indicator=rsi&ticker=BTC&interval=4h&period=14` (server proxy to TAAPI; supports aliases `close_10_ema`, `close_50_sma`, `close_200_sma`)
